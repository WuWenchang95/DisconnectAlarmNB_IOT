...
uint16_t AlarmStringPut(uint8_t *Sendbuff)
{
	...
	sprintf(stbuff,"{\"CODE\":\"%15s\",\"TYPE\":\"%s\"}\0",NbiotDev.sN,alarm_str[tPublishSt.alarmTpye]);
		...
		case NB_AT_NSOSD:
			{  //{"CODE":"0852019020097961","TYPE":"DISCONNECT_WIRE"}
				...
				if(NbiotDev.cmdTxStep == 0)
				{
					...
					//uint8_t stbuff[100] = {0};
					//uint8_t dtbuff[150] = {0};
					uint8_t xdata SendAtbuff[150] = {0};
					
					//sprintf(stbuff,"{\"SN\":\"%15s\",\"TYPE\":\"%s\"}\0",NbiotDev.sN,alarm_str[NbiotDev.alarmTpye]);
					
					//AsciiToHex(stbuff,strlen(stbuff),dtbuff);
					
					//sprintf(SendAtbuff,"AT+NSOSD=1,%d,%s\r\n\0",strlen(dtbuff),dtbuff);

					AlarmStringPut(SendAtbuff);
...
//			if(tPublishSt.isPublish == 0) && (tAlarmDri == 0)
//			{
//				EnterStopMode();
