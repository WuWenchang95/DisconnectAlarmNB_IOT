...
void EXINT8_ISR (void) interrupt 13 	 
{
	...
	if(RTCIF & RTC_AF) //闹钟中断
	{
		...
		tVoltageAdc.rtmWakeAdcFg = 1;
		tVoltageAdc.tCnt = 40;
		tVoltageAdc.lowFlag = 0;
		if(tVoltageAdc.publishFg)
		{
			tVoltageAdc.cleckCnt++;
			if(tVoltageAdc.cleckCnt > 5)
			{
				tVoltageAdc.cleckCnt = 0;
				tVoltageAdc.publishFg = 0;	
			}
		}
...
void RTM_init(unsigned char _hour, unsigned char _minute, unsigned char _sec)
{
	RTAH = _hour; //设置闹钟小时
	RTAM = _minute; //设置闹钟分
	RTAS = _sec; //设置闹钟秒
